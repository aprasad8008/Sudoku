...
public class Puzzle {
    ...
    //accounts for row and val 
    //accounts for col and val
    private boolean[][] row;
    private boolean[][] col;
    ...
    public Puzzle() {
        ...
        // fields that you add.
        row = new boolean[DIM][DIM+1];
        col = new boolean[DIM][DIM+1];
    ...
    public boolean solve(int n) {
        ...
        // of the method.
        if (n == (DIM*DIM)) {// base case: a solution!
            return true;
        }
        //initial row and col
        int row = n / 9;
        ...
        // when the value is not fixed and can check for if placing value in a row and col is safe
        for (int val = 1; val < DIM+1; val++) 
        {
            if (isSafe(row, col, val))
            {
                //can place and increment
    ...
    //check for safe move; when no value in row and col or subgridHasValue 
    public boolean isSafe (int row, int col, int val)
    {
        if (!this.row[row][val] && !this.col[col][val] && !subgridHasValue[row/SUBGRID_DIM][col/SUBGRID_DIM][val])
        {
            return true;
        }
        return false;
